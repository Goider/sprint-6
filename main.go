...
func (w Walking) Calories() float64 {
	...
	pow := math.Pow(speedMsec, 2)
	multWeight := CaloriesWeightMultiplier * w.Weight
	Heightm := w.Height / CmInM
	return (multWeight + (pow/Heightm)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
